...
import com.tsdl.common.sdk.base.BaseView;
import com.tsdl.common.sdk.sound.BaseSoundEffect;
import com.tsdl.common.sdk.sound.BaseSoundUtils;
import com.tsdl.common.sdk.sound.SoundEffectEnum;
...
public class BaseTabItemLayout extends BaseView implements ITabItem {
    ...
    public static final String TAG = BaseTabItemLayout.class.getSimpleName();
     ...
     */
    private static final int DEFAULT_OUTER_COLOR = R.color.cbox_mark_unchecked_color;
     ...
     */
    private static final int DEFAULT_TEXT_COLOR = R.color.assist_content_color;

    /**
     * default the disenable whole text color.
     */
    private static final int DEFAULT_DISABLE_TEXT_COLOR = R.color.segmented_default_text_color;

    /**
     * default the item text color.
     */
    private static final int DEFAULT_SELECTED_TEXT_COLOR = R.color.main_content_color;

    /**
     * default the item text size.
     */
    private static final int DEFAULT_SELECTED_TEXT_SIZE = R.dimen.segmented_selected_text_dimen;

    /**
     * default the whole text size.
     */
    private static final int DEFAULT_TEXT_SIZE = R.dimen.segmented_normal_text_dimen;
    ...
    public BaseTabItemLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mTargetResourceValues = AttributeHelper.readAttributeValues(context, attrs,
                R.styleable.BaseTabItemLayout, defStyleAttr, defStyleRes);
        init(context, attrs);
        if (mSoundEffect == BaseSoundUtils.SOUND_DEFAULT) {
            mSoundEffect = SoundEffectEnum.FX_KEY_NORMAL_SEGMENT.value();
        }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BaseTabItemLayout);
        if (ta == null) {
            return;
        }

        mHandler = new H();
        mIsOnClickBeforeAnimation = ta.getBoolean(R.styleable.BaseTabItemLayout_tabItemOnClickBeforeAnimation,
                true);
        mRadius = ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemRadius,
                dip2px(getContext(), DEFAULT_RADIUS));

        mOuterColor = ta.getColor(R.styleable.BaseTabItemLayout_tabItemOuterColor,
                ContextCompat.getColor(getContext(), DEFAULT_OUTER_COLOR));

        mItemColor = ta.getColor(R.styleable.BaseTabItemLayout_tabItemItemColor,
                ContextCompat.getColor(getContext(), DEFAULT_ITEM_COLOR));

        mDisableTextColor = ta.getColor(R.styleable.BaseTabItemLayout_tabItemDisableTextColor,
                ContextCompat.getColor(getContext(), DEFAULT_DISABLE_TEXT_COLOR));

        mSelectedTextColor = ta.getColor(R.styleable.BaseTabItemLayout_tabItemSelectedTextColor,
                ContextCompat.getColor(getContext(), DEFAULT_SELECTED_TEXT_COLOR));

        mSelectedItem = ta.getInteger(R.styleable.BaseTabItemLayout_tabItemSelectedItem,
                0);

        mAddItems = ta.getTextArray(R.styleable.BaseTabItemLayout_tabItemAddItems);

        mEnabled = ta.getBoolean(R.styleable.BaseTabItemLayout_tabItemEnabled, true);

        mTextSize = ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemTextSize,
                getResources().getDimensionPixelSize(DEFAULT_TEXT_SIZE));

        mTextColor = ta.getColor(R.styleable.BaseTabItemLayout_tabItemTextColor,
                ContextCompat.getColor(getContext(), DEFAULT_TEXT_COLOR));

        mSelectTextSize =
                ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemSelectTextSize,
                        getResources().getDimensionPixelSize(DEFAULT_SELECTED_TEXT_SIZE));

        mMode = ta.getInt(R.styleable.BaseTabItemLayout_tabItemMode, ROUND);

        mOrientation = ta.getInt(R.styleable.BaseTabItemLayout_tabItemOrientation, HORIZONTAL);
        mRedPointPosition = ta.getInteger(R.styleable.BaseTabItemLayout_tabItemRedPointPosition, 0);
        mRedPointShow = ta.getBoolean(R.styleable.BaseTabItemLayout_tabItemRedPointShow, false);
        mIsChild = ta.getBoolean(R.styleable.BaseTabItemLayout_tabItemChildEnable, false);
        if (mIsChild) {
            mOrientation = HORIZONTAL;
        }
        mItemMargin = getResources()
                .getDimensionPixelSize(R.dimen.tabItem_item_margin) / mHalfValue;
        mItemShadowMargin = getResources()
                .getDimensionPixelSize(R.dimen.tabItem_item_shadow_margin);

        mTextAndImagePadding = ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemTextAndImagePadding,
                getResources().getDimensionPixelSize(R.dimen.tabItem_text_image_padding));

        if (mOrientation == VERTICAL) {
            mPaddingStart =
                    ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemMarginTop,
                            getResources().getDimensionPixelSize(R.dimen.tabItem_vertical_padding));
            mPaddingSide =
                    ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemMarginLeft,
                            getResources().getDimensionPixelSize(R.dimen.tabItem_vertical_top_bottom_padding));
        } else {
            mPaddingStart =
                    ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemMarginLeft,
                            getResources().getDimensionPixelSize(R.dimen.tabItem_horizontal_left_padding));
            mPaddingSide =
                    ta.getDimensionPixelSize(R.styleable.BaseTabItemLayout_tabItemMarginLeft,
                            getResources().getDimensionPixelSize(R.dimen.tabItem_horizontal_padding));
        }
        LogUtils.logD(TAG, "init: mPaddingStart = " + mPaddingStart + ", mItemMargin = " + mItemMargin);
        mScrollEnable = ta.getBoolean(R.styleable.BaseTabItemLayout_tabItemScrollEnable,
                false);
        mDefaultBitmap = ta.getBoolean(R.styleable.BaseTabItemLayout_tabItemDefaultBitmap,
                true);
        mBgDrawable = ta.getDrawable(R.styleable.BaseTabItemLayout_tabItemBgDrawable);
        ...
        mItemDrawable = ta.getDrawable(R.styleable.BaseTabItemLayout_tabItemItemDrawable);
        checkDrawable = ta.getDrawable(R.styleable.BaseTabItemLayout_checkedItemColor);
        if (!mIsChild && mOrientation == HORIZONTAL) {
            mItemDrawable = ta.getDrawable(R.styleable.BaseTabItemLayout_tabItemTopBgDrawable);
            checkDrawable = ta.getDrawable(R.styleable.BaseTabItemLayout_topCheckDrawable);
        }
        if (mOrientation == VERTICAL) {
        ...
        if (mBgDrawable != null) {
            if (mOrientation == VERTICAL) {
                mBgDrawableTintList =
                        ta.getColorStateList(R.styleable.BaseTabItemLayout_tabItemNormalBackgroundTint);
                mBgDrawable.setTintList(mBgDrawableTintList);
            } else {
                mBgDrawableTopTintList =
                        ta.getColorStateList(R.styleable.BaseTabItemLayout_tabItemTopBackgroundTint);
        ...
        if (checkDrawable != null) {
            if (!mIsChild && mOrientation == HORIZONTAL) {
                mTopCheckDrawableTintList = ta.getColorStateList(R.styleable.BaseTabItemLayout_tabItemTopCheckTint);
                checkDrawable.setTintList(mTopCheckDrawableTintList);
            } else {
                mCheckDrawableTintList = ta.getColorStateList(R.styleable.BaseTabItemLayout_tabItemCheckTint);
        ...
        if (mOrientation == VERTICAL) {
            ...
            if (getItemWidth() == 0) {
                if (mItemType == ICON_TEXT) {
                    mWidth = getResources().getDimensionPixelSize(R.dimen.tabItem_default_width);
                } else {
            ...
            if (mItemType == ICON_TEXT) {
                itemHeight = getResources().getDimensionPixelSize(R.dimen.tabItem_vertical_default_height);
            } else {
            ...
            if (getItemWidth() == 0) {
                if (mIsChild) {
                    ...
                    if (mItemType == ICON_TEXT) {
                        itemWidth = getResources().getDimensionPixelSize(R.dimen.tabItem_default_width);
                    } else {
            ...
            mHeight = getResources().getDimensionPixelSize(R.dimen.tabItem_default_height);
            ...
            if (getType(i) == BaseTabItem.ITEMSTRING) {
                ...
                if (getItemEnable(i)) {
                    if (i == mSelectedItem) {
                        ...
                        if (mOrientation == HORIZONTAL && !mIsChild) {
                            mTextPaint.setColor(getContext().getColor(R.color.common_black));
                        }
                    } else {
                        mTextPaint.setColor(mTextColor);
                    }
...
//            mPaint.setColor(mItemColor);
     ...
     *
     * @param list BaseSegmentedControlItem
     */
     ...
     * add view item.
     *
     * @param item BaseSegmentedControlItem
     */
    ...
    public void setIsChildType(Boolean childType) {
        mIsChild = childType;
        if (mIsChild && mOrientation == HORIZONTAL) {
            int checkBgDrawableResId = mTargetResourceValues.get(R.attr.tabItemVerticalItemDrawable);
...
//        mSegmentedControlItems.forEach(new Consumer<BaseTabItem>() {
//            @Override
//            public void accept(BaseTabItem BaseSegmentedControlItem) {
//                float textSize = BaseSegmentedControlItem.getTextSize();
//                float newTextSize = textSize * newConfig.fontScale / oldFontScale;
//                BaseSegmentedControlItem.setTextSize(newTextSize);
//
//                float selectTextSize = BaseSegmentedControlItem.getSelectTextSize();
//                float newSelectTextSize = selectTextSize * newConfig.fontScale / oldFontScale;
//                BaseSegmentedControlItem.setSelectTextSize(newSelectTextSize);
//            }
//        });
//
...
//    }
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        int checkTintResId;
        if (!mIsChild && mOrientation == HORIZONTAL) {
            topBgDrawableResId = mTargetResourceValues.get(R.attr.tabItemTopBgDrawable);
            if (AttributeHelper.isValid(topBgDrawableResId)) {
                mBgDrawable = getResources().getDrawable(topBgDrawableResId, getContext().getTheme());
            }
            checkDrawableResId = mTargetResourceValues.get(R.attr.topCheckDrawable);
            ...
            bgTintResId = mTargetResourceValues.get(R.attr.tabItemNormalBackgroundTint);
            ...
            checkTintResId = mTargetResourceValues.get(R.attr.tabItemCheckTint);
            ...
            checkBgDrawableResId = mTargetResourceValues.get(R.attr.tabItemVerticalItemDrawable);
            if (AttributeHelper.isValid(checkBgDrawableResId)) {
                mItemDrawable = getResources().getDrawable(checkBgDrawableResId, getContext().getTheme());
            }
        }

        mTextColor = getContext().getColor(R.color.assist_content_color);
        mSelectedTextColor = getContext().getColor(R.color.main_content_color);
        invalidate();
    }
