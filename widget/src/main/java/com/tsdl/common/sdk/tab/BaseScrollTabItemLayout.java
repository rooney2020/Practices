...
import com.tsdl.common.sdk.base.BaseRelativeLayout;
import com.tsdl.common.sdk.scroll.BaseHorizontalScrollView;
import com.tsdl.common.sdk.scroll.BaseVerticalScrollView;
...
public class BaseScrollTabItemLayout extends BaseRelativeLayout {
    public static final String TAG = BaseScrollTabItemLayout.class.getSimpleName();
    ...
    private BaseVerticalScrollView BaseVerticalScrollView;
    private BaseHorizontalScrollView BaseHorizontalScrollView;
    ...
    private void init(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.BaseScrollTabItemLayout);
        mOrientation = a.getInt(R.styleable.BaseScrollTabItemLayout_scrollTabItemOrientation, HORIZONTAL);
        mIsChild = a.getBoolean(R.styleable.BaseScrollTabItemLayout_scrollTabItemChildEnable, false);
        overMode = a.getInt(R.styleable.BaseScrollTabItemLayout_scrollTabItemOverScrollMode, mHalfValue);
        mItemType = a.getInt(R.styleable.BaseScrollTabItemLayout_scrollTabItemItemType, TEXT);
        mItemWidth = a.getDimensionPixelSize(R.styleable.BaseScrollTabItemLayout_scrollTabItemItemWidth, 0);
        mRedPointPosition = a.getInteger(R.styleable.BaseScrollTabItemLayout_scrollTabItemRedPointPosition, 0);
        mRedPointShow = a.getBoolean(R.styleable.BaseScrollTabItemLayout_scrollTabItemRedPointShow, false);
        if (mOrientation == VERTICAL) {
            LayoutInflater.from(getContext()).inflate(R.layout.layout_vertical_tabitem, this, true);
            BaseVerticalScrollView = findViewById(R.id.tab_vertical_layout);
            tabItemLayout = findViewById(R.id.tab_vertical_item_layout);
            BaseVerticalScrollView.setOverScrollMode(overMode);
            BaseVerticalScrollView.setVerticalScrollBarEnabled(false);
        } else {
            LayoutInflater.from(getContext()).inflate(R.layout.layout_horizontal_tabitem, this, true);
            BaseHorizontalScrollView = findViewById(R.id.tab_horizontal_layout);
            tabItemLayout = findViewById(R.id.tab_horizontal_item_layout);
            BaseHorizontalScrollView.setOverScrollMode(overMode);
            BaseHorizontalScrollView.setHorizontalScrollBarEnabled(false);
        }
        if (mItemWidth != 0) {
        ...
        if (mOrientation == VERTICAL) {
            mTextSize = a.getDimensionPixelSize(R.styleable.BaseScrollTabItemLayout_scrollTabItemTextSize,
                    getResources().getDimensionPixelSize(defaultVerticalSize));
            mSelectTextSize =
                    a.getDimensionPixelSize(R.styleable.BaseScrollTabItemLayout_scrollTabItemSelectTextSize,
                            getResources().getDimensionPixelSize(defaultVerticalSize));
        } else {
            mTextSize = a.getDimensionPixelSize(R.styleable.BaseScrollTabItemLayout_scrollTabItemTextSize,
        ...
        if (mOrientation == VERTICAL) {
            mPaddingSide =
                    a.getDimensionPixelSize(R.styleable.BaseScrollTabItemLayout_scrollTabItemMarginLeft,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            BaseVerticalScrollView.setBackgroundResource(R.drawable.img_tabitem_side_bg);
        } else {
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        if (mOrientation == VERTICAL) {
            BaseVerticalScrollView.setBackground(getContext().getDrawable(R.drawable.img_tabitem_side_bg));
        }
        if (tabItemLayout != null) {
