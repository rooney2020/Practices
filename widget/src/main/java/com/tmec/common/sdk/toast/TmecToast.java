package com.tmec.common.sdk.toast;
...
import com.tmec.common.sdk.textview.TmecTextExplainBody;

public class TmecToast extends Toast implements SkinManager.OnSkinModeChangeListener {
    ...
    private static final int MARGIN_296PX = 296;
    private static final int TOAST_HEIGHT_150PX = 150;
    private static final int TYPE_TEXT_MAX_WIDTH = 1084;
    private static final int TOAST_MAX_WIDTH = 1300;
    private static final int TYPE_ICON_MAX_WIDTH = 1004;
    ...
    private static void setToastWidth() {
        ...
        int textWidth = (int) mTextView.getPaint().measureText(mMessageTv);
        int bottomTvWidth = (int) mDoubleBottomTv.getPaint().measureText(mMessageTv);
        int width = mTypeToast == TYPE_TEXT ? MARGIN_216PX + textWidth : MARGIN_296PX + textWidth;
        int bottomWidth = mTypeToast == TYPE_TEXT ? MARGIN_216PX + bottomTvWidth : MARGIN_296PX
                + bottomTvWidth;
        setMaxWidth(textWidth, width, bottomWidth, llParams);
        mToastContentRl.setLayoutParams(llParams);
    }

    private static void setMaxWidth(int textWidth, int width, int bottomWidth,
                                    RelativeLayout.LayoutParams params) {
        if (mTypeToast == TYPE_TEXT) {
            mTextView.setMaxWidth(TYPE_TEXT_MAX_WIDTH);
            int paramsWidth = textWidth > TYPE_TEXT_MAX_WIDTH ? TOAST_MAX_WIDTH : width;
            params.width = paramsWidth;
        } else if (mTypeToast == TYPE_SINGLE_ICON) {
            mTextView.setMaxWidth(TYPE_ICON_MAX_WIDTH);
            int maxWidth = textWidth > TYPE_ICON_MAX_WIDTH ? TOAST_MAX_WIDTH : width;
            params.width = maxWidth;
        } else if (mTypeToast == TYPE_DOUBLE_ICON) {
            mTextView.setMaxWidth(TYPE_ICON_MAX_WIDTH);
            mDoubleBottomTv.setMaxWidth(TYPE_ICON_MAX_WIDTH);
            int maxWidth = textWidth > TYPE_ICON_MAX_WIDTH ? TOAST_MAX_WIDTH : width;
            int maxBottomWidth = bottomWidth > TYPE_ICON_MAX_WIDTH ? TOAST_MAX_WIDTH : bottomWidth;
            int paramsWidth = maxWidth > maxBottomWidth ? maxWidth : maxBottomWidth;
            params.width = paramsWidth;
        }
    }
    ...
    private static void updateSkinChanged() {
        mToastContentRl.setBackground(SkinManager.Singleton.getInstance().isNight()
        ...
        mToastProBar.setIndeterminateDrawable(SkinManager.Singleton.getInstance().isNight()
